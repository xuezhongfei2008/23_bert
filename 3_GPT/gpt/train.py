...
def model(X, M, Y, train=False, reuse=False):
    with tf.variable_scope('model', reuse=reuse):
        ...
        # n_ctx 应该是 n_context,max_len
        we = tf.get_variable("we", [n_vocab+n_special+n_ctx, n_embd], initializer=tf.random_normal_initializer(stddev=0.02))
        we = dropout(we, embd_pdrop, train)
        # print("X1",X)
        '''X:(batch_size,2[comp1,comp2],max_len,2[wordID,posID])'''
        X = tf.reshape(X, [-1, n_ctx, 2])
        '''X:[batch_size*2,max_len,2-(wordID,posID)]将batch与comp合并成一个维度'''
        print("M",M)
        '''M:[batch_size,2-(comp1,comp2),max_len]'''
        M = tf.reshape(M, [-1, n_ctx])
        '''M:[batch_size*2,max_len]'''
        ...
        for layer in range(n_layer):
            #h_l=transformer_b lock(h_l-1)
            h = block(h, 'h%d'%layer, train=train, scale=True)
            '''h:[batch_size*2,max_len,n_embd]'''
        print("h",h)
        # 3. 计算语言模型loss

        lm_h = tf.reshape(h[:, :-1], [-1, n_embd])
        '''h[:, :-1] 得到的维度[batch_size*2,max_len-1,n_embd]去掉了clf_token 这个字，再映射成[batch_size*2*(max_len-1),n_emdb]'''
        # print("lm_h",lm_h)
        '''P(u)=softmax(h_n WeT)'''
        lm_logits = tf.matmul(lm_h, we, transpose_b=True)
        '''lm_logits:[batch_size*2*(max_len-1),n_vocab] 其实就是每个输入字对应n_vocab的概率值'''
        # print("lm_logits",lm_logits)
        lm_losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=lm_logits, labels=tf.reshape(X[:, 1:, 0], [-1]))
        '''X[:, 1:, 0]就是[batch_size*2,max_len-1,1-(wordID)]当前字预测下一个字，如输入：我是中国人，则有
        start-我
        我-是
        是-中国人
        中国人-end
        ...
        '''映射成[batch_size*2,max_len-1]'''
        print("lm_losses",lm_losses)
        lm_losses = tf.reduce_sum(lm_losses*M[:, 1:], 1)/tf.reduce_sum(M[:, 1:], 1)
        '''M[:,1:]去除start这个字符'''
        print("lm_losses2", lm_losses)
        # 4. 计算classifier loss
        clf_h = tf.reshape(h, [-1, n_embd])
        '''clf_h:[batch_size*2*max_len,n_embd]'''
        print("clf_h",clf_h)
...
if __name__ == '__main__':
    ...
    '''trX:[batch_size,2:(comps1,comps2),max_len,2:(wordID,posID)],2分别是两个维度，分别入（）所示'''
