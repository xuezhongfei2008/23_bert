...
flags.DEFINE_string(
    "data_dir", os.path.join(root_path, 'NERdata/bak'),
    "The input datadir.",
)
...
class NerProcessor(DataProcessor):
    ...
    def _create_example(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            '''
            guid train-5
            text 克 马 尔 的 女 儿 让 娜 今 年 读 五 年 级 ， 她 所 在 的 班 上 有 3 0 多 名 同 学 ， 该 班 的 “ 家 委 会 ” 由 1 0 名 家 长 组 成 。
            label B-PER I-PER I-PER O O O B-PER I-PER O O O O O O O O O O O O O O O O O O O O O O O O O B-ORG I-ORG I-ORG O O O O O O O O O O   
          '''
...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    embedding = model.get_sequence_output()   #transformers最后一层的输出
