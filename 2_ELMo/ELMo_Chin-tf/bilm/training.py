...
class LanguageModel(object):
    ...
    def _build_word_char_embeddings(self):
        ...
        with tf.device("/cpu:0"):
            ...
            print("self.embedding_weights",self.embedding_weights)
            print("self.tokens_characters",self.tokens_characters)
    ...
    def _build(self):
        ...
        for lstm_num, lstm_input in enumerate(lstm_inputs):
            print("aaalstm_inputs",lstm_inputs)
            lstm_cells = []
        ...
        print("bbbblstm_outputs",lstm_outputs)
...
def train(options, data, n_gpus, tf_save_dir, tf_log_dir,
    ...
    with tf.device('/cpu:0'):
        ...
        # print("sssssssssssssslstm_out",lstm_out)
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        # print("hhhhhhhhhhhhhdata_gen",data_gen)
        for batch_no, batch in enumerate(data_gen, start=1):
            ...
            if batch_no == n_batches_total:
                ...
                embed=sess.run(model.embedding_weights)  #获取所有的embedding_weights,numpy
                with h5py.File('../log/vocab_weights.hdf5','w') as fout:
                    fout.create_dataset('embedding',embed.shape,dtype='float32',data=embed)
                # done training!
                break

def train_with_cpu(options, data, tf_save_dir, tf_log_dir,
          restart_ckpt_file=None):
    if restart_ckpt_file is None:
        with open(os.path.join(tf_save_dir, 'options.json'), 'w') as fout:
            fout.write(json.dumps(options))
    ...
    with tf.variable_scope('lm', reuse=False):
        ...
        loss = model.total_loss
        models.append(model)
        grads = opt.compute_gradients(loss*options['unroll_steps'],
                                      aggregation_method=tf.AggregationMethod.EXPERIMENTAL_TREE)
        tower_grads.append(grads)
        train_perplexity += loss
    print_variable_summary()
    ...
    grads, norm_summary_ops = clip_grads(grads, options, True, global_step)
    norm_summaries.extend(norm_summary_ops)
    train_perplexity = tf.exp(train_perplexity)
    ...
    histogram_summaries.append(tf.summary.histogram(
        'lstm_embedding_0', lstm_out[0]
    ))
    if options.get('bidirectional', False):
        histogram_summaries.append(tf.summary.histogram(
            'lstm_embedding_1', lstm_out[1]
        ))
    train_op = opt.apply_gradients(grads, global_step=global_step)
    for v in tf.global_variables():
        print("ddddd",v)
        print("dddddddv",v.name)
        histogram_summaries.append(tf.summary.histogram(
            v.name.replace(':', '_'), v
        ))
    histogram_summaries.extend(summary_gradient_updates(
        grads, opt, lr
    ))
    saver = tf.train.Saver(tf.global_variables(), max_to_keep=2)
    summary_op = tf.summary.merge(
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        batch_size = options['batch_size']
        unroll_steps = options['unroll_steps']
        n_train_tokens = options.get('n_train_tokens', 111111)
        n_tokens_per_batch = batch_size * unroll_steps * 1
        n_batches_per_epoch = n_train_tokens // n_tokens_per_batch
        n_batches_total = options['n_epochs'] * n_batches_per_epoch
        print('Training for {} epochs and {} batches'.format(
            options['n_epochs'], n_batches_total
        ))
        init_state_tensors = []
        final_state_tensors = []
        ...
        init_state_values = sess.run(init_state_tensors, feed_dict=feed_dict)
        t1 = time.time()
        data_gen = data.iter_batches(batch_size * 1, unroll_steps)
        for batch_no, batch in enumerate(data_gen, start=1):
...
def train_with_single_core(options, data, tf_save_dir, tf_log_dir,
                           restart_ckpt_file=None):
    if restart_ckpt_file is None:
        with open(os.path.join(tf_save_dir, 'options.json'), 'w') as fout:
            fout.write(json.dumps(options))
        ...
        with sv.managed_session(master='') as sess:
            ...
            for batch_no, batch in enumerate(data_gen, start=1):
                ...
                try:
                    ...
                    sv.saver.save(
                        sess, tf_log_dir + '/model', global_step=(batch_no + 1))
                    raise e
