...
class Vocabulary(object):
    ...
    def id_to_word(self, cur_id):
        if cur_id >= len(self._id_to_word):
            return '<UNK>'
...
class UnicodeCharsVocabulary(Vocabulary):
    ...
    def __init__(self, filename, max_word_length, **kwargs):
        ...
        filename1 = '../data/vocab_seg_words_elmo.txt'
        self.char_vocab = Vocabulary(filename1, True)
        # char ids 0-255 come from utf-8 encoding bytes
        # assign 256-300 to special chars
        max_pos = len(self._word_to_id)
        self.bos_char = max_pos + 1  # <begin sentence>
        self.eos_char = max_pos + 2  # <end sentence>
        self.bow_char = max_pos + 3  # <begin word>
        self.eow_char = max_pos + 4  # <end word>
        self.pad_char = max_pos + 5 # <padding>
    ...
    def _convert_word_to_char_ids(self, word):
        ...
        for k, chr_id in enumerate(word_encoded, start=1):
            code[k] = self.char_vocab._word_to_id[chr_id] if chr_id in self.char_vocab._word_to_id else self.char_vocab.unk
